...
{
    std::lock_guard<std::mutex> lock(stateMutex);
    stored.selection = std::clamp(f, 0, familyCount - 1);
}
...
bool NeuroProcessor::factory(int i)
{
    if (i < 0 || i >= presetCount)
        return false;
    std::lock_guard<std::mutex> lock(stateMutex);
    return commitPatch(factoryPatch(i));
}
juce::ValueTree NeuroProcessor::snapshotLocked()
{
    auto root = juce::ValueTree("BREZEBASS");
...
{
    if (!root.hasType("BREZEBASS") || int(root.getProperty("version", 0)) != 3)
        return false;
    ...
    p.family = std::clamp(int(root.getProperty("family", 1)), 0, familyCount - 1);
    p.calibration = clamp(float(root.getProperty("calibration", 1)), .55f, 2);
    p.selection = std::clamp(int(root.getProperty("selection", p.family)), 0, familyCount - 1);
