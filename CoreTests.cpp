...
static void renderDemo(const char *path)
{
    ...
    e->prepare(48000);
    for (int family : {30, 3, 4, 9, 8, 6, 10, 29})
    {
        auto p = nj::factoryPatch((family - 1) * 5 + 2);
        e->reset();
...
int main(int argc, char **argv)
{
    try
    {
        ...
        std::array<int, familyCount> counts{};
        for (int index = 0; index < presetCount; ++index)
        {
            auto p = factoryPatch(index);
            ++counts[p.family];
            e->reset();
            e->tables.build(p.waveSeed);
            e->set(p.v, true);
            ...
            {
                auto y = e->tick();
                require(std::isfinite(y[0]) && std::abs(y[0]) <= .913f, "Factory output unsafe");
                power += y[0] * y[0];
            }
            require(power > 1e-6, "Factory preset silent");
        }
        for (int family = 1; family < familyCount; ++family)
            require(counts[family] == 5, "Not five presets per family");
        auto syncValues = defaults();
        syncValues[GateSpeed] = syncValues[LFOSpeed] = 2;
        syncValues[GateDepth] = .8f;
        e->prepare(48000);
        e->bpm = 120;
        e->set(syncValues, true);
        e->noteOn(36, 1);
        for (int i = 0; i < 19200; ++i)
            e->tick();
        require(std::abs(e->voices[0].gatePhase - .6f) < .002f && std::abs(e->voices[0].lfo1 - .6f) < .002f,
                "Gate/LFO BPM sync failed");
        e->reset();
        e->bpm = 60;
        e->set(syncValues, true);
        e->noteOn(36, 1);
        for (int i = 0; i < 19200; ++i)
            e->tick();
        require(std::abs(e->voices[0].gatePhase - .8f) < .002f, "Gate does not follow tempo");
        e->reset();
        e->bpm = 120;
        syncValues[Sync] = 0;
        e->set(syncValues, true);
        ...
        for (int i = 0; i < 19200; ++i)
            e->tick();
        require(std::abs(e->voices[0].gatePhase - .8f) < .002f && std::abs(e->voices[0].lfo1 - .8f) < .002f,
                "Hz mode still follows tempo");
        ...
        std::cout << "PASS: deterministic patches/tables; every section lock; FFT roundtrip and STFT "
                     "latency/reconstruction; 30 families x 4 rates; 200 extra seeds; extreme 8-voice patch; "
                     "RMS calibration; safe output; note-off; sustain; mono fallback; block partition "
                     "invariance; invalid parameter repair.\n";
        std::cout << "PASS: 150 factory presets; exactly five in each family; gate/LFO shared BPM and Hz "
                     "switching.\n";
