...
inline Patch generate(Patch base, uint32_t seed, int type, bool mutate = false)
{
    ...
    Patch p = base;
    p.seed = seed;
    p.selection = std::clamp(type, 0, familyCount - 1);
    p.family = type == 0 ? 1 + r.pick(familyCount - 1) : std::clamp(type, 1, familyCount - 1);
    auto &v = p.v;
    ...
    else
    {
        ...
        set(PhaserMix, r.range(.1f, .55f) * complex);
        set(PhaserRate, r.range(.15f, 1.2f));
        if (f == 5 || f == 14 || f == 19)
        {
            set(VowelMix, .7f);
            set(Vowel, r.range(0, 4));
        }
        if (color || f == 29)
        {
            set(ColorDensity, float(4 + r.pick(5)));
            set(ColorSpread, r.range(.2f, 1.5f));
            set(ColorDetune, r.range(1, 10));
            set(ColorExcite, r.range(.7f, 2));
            set(ColorBrightness, r.range(3500, 14000));
        }
        if (spectral || f == 29)
        {
            set(Prism, r.range(-.5f, .5f));
            set(SpectralSmear, r.range(.1f, .65f));
            set(HarmonicMix, r.range(.1f, .65f));
            set(HarmonicInterval, r.pick(2) ? 7 : 12);
            set(HarmonicSnap, r.range(0, .5f));
        }
        if (f == 3 || f == 4 || f == 16 || f == 20)
        {
            set(BlownOn, 1);
            set(BlownMix, .35f);
            set(BlownDrive, r.range(3, 10));
            set(BlownSag, .25f);
        }
        if (f == 17 || f == 23 || f == 24 || f == 30)
        {
            set(GateDepth, r.range(.5f, .95f));
            set(GateSpeed, r.pick(2) ? 4 : 8);
            set(GateDuty, r.range(.3f, .7f));
        }
        if (f == 30)
        {
            set(FMAmount, r.range(2, 4));
            set(PMAmount, r.range(.5f, 1.6f));
            set(Ratio, r.pick(2) ? 3 : 5);
            set(VowelMix, .45f);
            set(PhaserMix, .65f);
            set(ColorOn, 1);
            set(ColorAmount, .45f);
            set(SpectralOn, 1);
            set(SpectralMix, .4f);
            set(Prism, r.range(-.2f, .35f));
            set(HarmonicMix, .35f);
            set(SpectralSmear, .25f);
            set(ModWT, .8f);
            set(BlownOn, 1);
            set(BlownDrive, 5);
            set(BlownMix, .25f);
...
inline Patch factoryPatch(int index)
{
    index = std::clamp(index, 0, presetCount - 1);
    int variant = index % 5;
    Patch base;
    base.v[Brightness] = .35f + .12f * variant;
    base.v[Destruction] = .35f + .14f * variant;
    base.v[Complexity] = .45f + .12f * variant;
    auto p = generate(base, 318000u + uint32_t(index) * 7919u, 1 + index / 5);
    p.v[LFOShape] = float(variant);
    p.v[ColorSpread] = .15f + .35f * variant;
    p.v[ColorDetune] = 2 + 3.f * variant;
    p.v[Prism] = (variant - 2) * .16f;
    p.v[HarmonicInterval] = variant % 2 ? 7 : 12;
    p.v[SpectralSmear] = variant == 2 ? .65f : .08f * variant;
    if (variant == 3)
    {
        p.v[BlownOn] = 1;
        p.v[BlownDrive] = 8;
