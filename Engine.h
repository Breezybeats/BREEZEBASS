...
struct Voice
{
    ...
    SVF filter;
    OnePole highpass;
    std::array<Resonator, 8> colors;
    std::array<SVF, 3> vowelFilters;
    OnePole colorTone;
    float gatePhase = 0, gateGain = 1;
              ...
              float bend, float wheel, float pressure, float &mid, float &sub)
    {
        ...
        gatePhase += p[GateSpeed] * sync / sr;
        gatePhase -= std::floor(gatePhase);
        const float gateTarget = gatePhase < p[GateDuty] ? 1.f : 1 - p[GateDepth];
        gateGain += (gateTarget - gateGain) * (1 - std::exp(-1 / (sr * p[GateSmooth] * .001f)));
        ...
        if (p[VowelMix] > .0001f)
        {
            static constexpr float vowels[5][3] = {
                {800, 1150, 2900}, {400, 1700, 2600}, {300, 2200, 3000}, {500, 900, 2600}, {350, 700, 2400}};
            int a = int(p[Vowel]), b = std::min(a + 1, 4);
            float f = p[Vowel] - a, formant = 0;
            for (int k = 0; k < 3; ++k)
            {
                float h = vowels[a][k] * (1 - f) + vowels[b][k] * f;
                h *= std::pow(2.f, movement * .2f);
                formant += vowelFilters[k].tick(hi, std::tan(pi * h / sr), 3, 1) * (k == 0 ? .65f : .25f);
            }
            x = x * (1 - p[VowelMix]) + formant * p[VowelMix];
            hi = x;
        }
        if (p[ColorOn] > .5f)
        {
            ...
            float sum = 0;
            const int density = int(p[ColorDensity]);
            for (int k = 0; k < density; ++k)
            {
                float tone =
                    std::max(120.f, hz * std::pow(2.f, (12 + p[ColorRoot] + chords[int(p[Chord])][k % 4] +
                                                        12 * (k / 4) * p[ColorSpread] +
                                                        (k - 3.5f) * p[ColorDetune] / 100) /
                                                           12.f));
                float delay = sr / tone * (1 + movement2 * p[ColorMotion] * .001f * (k - 1.5f));
                float fb = std::pow(.001f, 1 / (tone * p[ColorDecay]));
                sum +=
                    colors[k].tick(std::tanh(hi * p[ColorExcite]), delay, fb * (1 - p[ColorDamping] * .08f));
            }
            sum = colorTone.low(sum, 1 - std::exp(-tau * std::min(p[ColorBrightness], sr * .4f) / sr));
            x = x * (1 - p[ColorAmount] * .5f) + sum * p[ColorAmount] * 4.8f / density;
        }
        float rhythm = p[RhythmOn] > .5f ? p[Step0 + std::min(15, int(beat * 4))] : 1;
        float gain = env * velocity * rhythm * gateGain;
  ...
  public:
    ...
    std::array<float, 2> tick()
    {
        ...
        for (const auto &voice : voices)
            if (voice.active)
                spectralRoot = voice.hz;
        float spec = spectral.tick(mid, p, p[SpectralOn] > .5f, sr, spectralRoot);
        ...
        x = effects.tick(x, p, sr, spectralRoot);
        if (p[CleanSub] > .5f)
